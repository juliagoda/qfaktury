...
InvoiceRR::InvoiceRR(QWidget *parent, IDataLayer *dl, QString in_form): Invoice(parent, dl, in_form)
{
...
{
    additEdit->setText(trUtf8("Oświadczam, że jestem rolnikiem ryczałtowym zwolnionym od podatku od towarów"
...
void InvoiceRR::makeInvoice()
{
    ...
    {
        QMessageBox::information(this, "QFaktury", trUtf8("Zaznacz opcję \"jest płatnikiem VAT\" dla wybranego przedsiębiorcy"),
                QMessageBox::Ok);
        return;
    }
...
void InvoiceRR::setData(InvoiceData &invData) {
    ...
    if (ifpaysVAT->isChecked()) invData.ifpVAT = true;
    else invData.ifpVAT = false;
