...
// different values on different distributions, for me that was /usr/share/<APPNAME>
	...
	void checkSettings()
	{
		...
		if (value("units").toString().compare("") == 0)
		{
			setValue(
				"units",
				tr("szt|kg|g|m|km|godz|ar|bochenek|btl|cal|doba|egz|filiżanka|fracht|"
	...
	void resetSettings()
	{
		...
		setValue(
			"units",
			tr("szt|kg|g|m|km|godz|ar|bochenek|btl|cal|doba|egz|filiżanka|fracht|GJ|"
	...
	int getCustomerType(QString custType)
	{
		...
		if (custType.compare(trUtf8("Urząd")) == 0 || custType.compare(trUtf8("urząd")) == 0)
		{
			return 1;
		}
