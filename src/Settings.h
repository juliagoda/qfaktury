...
// /usr/share/<APPNAME>
  ...
  QString getAppDirs() { return appPath + "/"; }
  ...
  void resetSettings() {
    ...
    setValue("payments",
             trUtf8("gotówka|akredytywa|barter|karta kredytowa|karta "
                    "płatnicza|mieszany|przy odbiorze|ukryj na wydruku|za "
                    "pobraniem|zapłacono|zgodnie z umową|przelew|zaliczka"));
    setValue("pdfQuality", "1");
    setValue("corrections",
             trUtf8("zmiana ilości|zmiana waluty|zmiana rabatu|zmiana sposobu "
                    "płatności|zmiana kontrahenta|zmiana towaru/usługi|zmiana "
                    "daty terminu|zmiana daty sprzedaży"));
    setValue("prefix", "");
    setValue("renamed", "tak");
  ...
  QString getVersion(QString appName) {
    QString str = appName;
    str.truncate(2);
    return str.toUpper() + appName.right(6) + trUtf8(" - wersja ") +
           STRING(QFAKTURY_VERSION);
  }
  ...
  int getCustomerType(QString custType) {
    if (custType.compare(trUtf8("Firma")) == 0 ||
        custType.compare(trUtf8("firma")) == 0) {
      return 0;
    } else if (custType.compare(trUtf8("Urząd")) == 0 ||
               custType.compare(trUtf8("urząd")) == 0) {
      return 1;
    } else {
      return 2;
    }
  ...
  QString getNaturalPerson() { return QString("person"); }

  QString getOfficeName() { return QString("office"); }

  QString getCompanyNameTr() { return trUtf8("Firma"); }
