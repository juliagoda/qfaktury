...
// "This virtual function is called by QWizard::validateCurrentPage() when the user clicks Next or
// Finish to perform some last-minute validation.
// If it returns true, the next page is shown (or the wizard finishes); otherwise, the current page
// stays up." From http://doc.qt.io/qt-5/qwizardpage.html#validatePage
	...
	: QWizardPage(parent)
{
	...
	setSubTitle(
		tr("Wybierz swój host po lewej stronie poniżej, do którego należy twoja skrzynka pocztowa, "
		   "a z której chcesz wysłać wiadomość do kontrahenta."
		   " Po prawej wybierz jeden z portów. W przypadku zaznaczenia nieobsługiwanego portu dla "
		   "twojego konta pocztowego zostaniesz o tym poinformowany. Możesz także wprowadzić "
		   "własne dane bez zaznaczania."));
	...
	else if (checked == "Gazeta")
	{
		...
		else if (protocol == "TCP")
		{
			...
			QMessageBox::information(
				this,
				"Protokół",
				"Do wysłania wiadomości z tej skrzynki pocztowej wybierz protokół SSL.");
		}
		else if (protocol == "TLS")
		{
			this->protocol = "";
	...
	: QWizardPage(parent)
{
	...
	setSubTitle(
		tr("Wybierz jeden z szablonów do wygenerowania"
		   " treści lub wprowadź własną. Wybierz \"Zakończ\", żeby zamknąć okno. Pamiętaj, aby "
		   "przed zamknięciem wysłać wiadomość. Tymczasowo program po wysłaniu wiadomości kończy "
