...
MainWindow::~MainWindow()
{
	...
	//FIXME: investigate these emory leaks!
	if (!timer)
	{
		timer = nullptr;
	}
...
void MainWindow::init()
{
	...
	if (firstRun())
	{
		...
		if (QMessageBox::information(
				this,
				"QFaktury",
				trUtf8("Czy chcesz skonfigurować firmę? Opcja ta przy starcie programu będzie "
					   "widoczna tylko przy pierwszym uruchomieniu."),
				trUtf8("Tak"),
				trUtf8("Nie"),
				nullptr,
				0,
...
void MainWindow::checkTodayTask(QString whatToDo)
{
	...
	ui->todayExercise->setStyleSheet(
		"QTextEdit"
		"{padding-left: 10px;"
		"padding-right: 10px;"
		"padding-top: 30px;"
...
void MainWindow::pluginSlot()
{
	...
	{
		QMessageBox::information(
			this, trUtf8("QFaktury"), trUtf8("Uruchomienie się nie powiodło."), QMessageBox::Ok);
	}
...
void MainWindow::aboutProg()
{
	QMessageBox::about(
		...
		trUtf8("<h2>Program do wystawiania faktur</h2><br/><p>") + sett().getVersion(qAppName())
			+ trUtf8("<br/>Wymagane Qt >= 5.0.0")
			+ trUtf8("<br/>Kompilowane z Qt ")
			+ QT_VERSION_STR
			+ trUtf8("<br/>Twoja aktualna wersja - Qt ")
			+ qVersion()
			+ trUtf8("</p><p>Koordynator projektu: Grzegorz Rękawek</p>  <p>Programiści: Tomasz "
					 "Pielech, Rafał Rusin http://people.apache.org/~rr/, Sławomir Patyk, Jagoda "
					 "Górska</p>")
			+ trUtf8("<p>Ikony: Dariusz Arciszewski </p><p>Portowanie na Qt5: Jagoda \"juliagoda\" "
					 "Górska</p><br/>")
			+ trUtf8("<p>Testy w środowisku Arch Linux: Piotr \"sir_lucjan\" Górski && Paweł "
					 "\"pavbaranov\" Baranowski</p>")
			+ trUtf8("<h2><center>UWAGA!!!</center></h2>")
			+ trUtf8("<p align=\"justify\">Ten program komputerowy dostarczany jest przez autora w "
					 "formie \"takiej, jaki jest\". ")
			...
			+ trUtf8("użytkownika. Autor nie odpowiada za jakiekolwiek straty wynikające z "
					 "użytkowania ")
...
void MainWindow::editFHist()
{
	...
	if (ui->tableH->item(row, 3)->text() == trUtf8("korekta"))
	{
		...
		Correction corWindow(this, dl, s_WIN_CORRECT_EDIT, true);
	...
	{
		// QMessageBox::information( this, trUtf8("QFaktury"), "Jeszcze nie ma", QMessageBox::Ok );
		CorrectGross corWindow(this, dl, s_WIN_CORRECT_EDIT, true);
		corWindow.correctionInit(true);
		corWindow.readCorrData(ui->tableH->item(row, 0)->text());
	...
	{
		Bill billWindow(this, dl, s_BILL_EDIT);
	...
	{
		Invoice invWindow(this, dl, s_WIN_INVOICE_EDIT);
	...
	{
		InvoiceRR invWindow(this, dl, s_RR);
	...
	{
		Invoice invWindow(this, dl, s_WIN_PROFORMA_EDIT);
	...
	{
		InvoiceGross invWindow(this, dl, s_BR_INVOICE_EDIT);
	...
	{
		Duplicate dupWindow(this, dl, s_WIN_DUPLICATE_LOOK, true);
...
void MainWindow::buyerDel()
{
	if (ui->tableK->selectedItems().count() <= 0)
	{
		QMessageBox::information(
			this,
			trUtf8("QFaktury"),
			trUtf8("Kontrahent nie wybrany. Nie mozna usuwac."),
			trUtf8("Ok"),
			nullptr,
			nullptr,
			1);
...
void MainWindow::printBuyerList()
{
	...
	if (preview.exec() == 1)
	{
		QMessageBox::warning(
			this,
			trUtf8("Drukowanie"),
			trUtf8("Prawdopobnie nie masz skonfigurowanej drukarki. Wykrywana nazwa domyślnej "
				   "drukarki to: ")
				+ printer.printerName()
				+ trUtf8(". Status domyślnej drukarki (poprawny o ile drukarka ma możliwość "
						 "raportowania statusu do systemu): ")
...
void MainWindow::printList(QPrinter *printer)
{
	...
	else
	{
		if (QMessageBox::warning(
				...
				trUtf8("Aby móc wydrukować listę kontaktów, musisz mieć wprowadzonego co najmniej "
					   "jednego kontrahenta do tabeli. Czy chcesz dodać teraz dane twojego "
					   "kontrahenta?"),
				trUtf8("Tak"),
				trUtf8("Nie"),
				nullptr,
...
{
	Invoice invWindow(this, dl, s_INVOICE);
...
{
	Bill billWindow(this, dl, s_BILL);
...
{
	InvoiceGross invWindow(this, dl, s_FBRUTTO);
...
{
	Invoice invWindow(this, dl, s_PROFORMA);
...
void MainWindow::newCor()
{
	if (ui->tableH->selectedItems().count() <= 0)
	{
		QMessageBox::information(
			this,
			trUtf8("QFaktury"),
			trUtf8("Faktura nie wybrana. Wybierz fakurę, do której chcesz wystawić korektę."),
			trUtf8("Ok"),
			nullptr,
			nullptr,
			1);
...
void MainWindow::newDuplicate()
{
	if (ui->tableH->selectedItems().count() <= 0)
	{
		QMessageBox::information(
			this,
			trUtf8("QFaktury"),
			trUtf8("Faktura nie wybrana. Wybierz fakurę, do której chcesz wystawić duplikat."),
			trUtf8("Ok"),
			nullptr,
			nullptr,
			1);
	...
	if (invTypes.contains(ui->tableH->item(row, 3)->text()))
	{
		Duplicate dupWindow(this, dl, s_DUPLICATE, false);
...
void MainWindow::noteDownTask(const QDate &taskDate)
{
	...
	QString planDir =
		QDir::homePath() + "/.local/share/data/elinux/plans/" + taskDate.toString() + ".txt";
	QFile filename(planDir);
...
{
	QFile file(
		QDir::homePath() + "/.local/share/data/elinux/plans/" + markedDate.toString() + ".txt");
	bool removed = false;
	...
	else
	{
		...
		else
		{
			QMessageBox::critical(
				this,
				trUtf8("Usuwanie zadań"),
				trUtf8("Zadania z wybranego dnia nie mogły zostać pomyślnie usunięte. Zrestartuj "
					   "program, by wyeliminować ewentualny brak aktualnych danych o plikach."),
				QMessageBox::Ok);
		}
	}
	...
	if (file.open(QIODevice::ReadWrite | QIODevice::Truncate | QIODevice::Text))
	{
		...
		else
		{
			...
			QMessageBox::critical(
				this,
				trUtf8("Dodawanie zadania"),
				trUtf8("Zadanie nie mogło zostac dodane. Sprawdź, czy istnieje ścieżka: ") + planDir
					+ trUtf8(" . Jeśli istnieje to sprawdź, czy masz uprawnienia do zapisu i "
							 "odczytu w podanej ścieżce."),
				QMessageBox::Ok);
		}
	}
	else
...
void MainWindow::addNextTask()
{
	...
	if (file.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
	{
		...
		if (stream.status() == QTextStream::Ok)
		{
			QMessageBox::information(
				this,
				trUtf8("Dopisywanie kolejnego zadania"),
			...
			if (check_file.exists() && check_file.isFile())
			{
				...
				else
				{
					QMessageBox::critical(
						this,
						trUtf8("Dopisywanie kolejnego zadania"),
						trUtf8("Dodatkowe zadanie nie mogło zostac dodane. Sprawdź, czy istnieje "
							   "ścieżka: ")
							+ planDir
							+ trUtf8(" . Jeśli istnieje to sprawdź, czy masz uprawnienia do zapisu "
									 "i odczytu w podanej ścieżce."),
						QMessageBox::Ok);
				}
			}
		}
