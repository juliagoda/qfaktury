...
MainWindow::MainWindow(QWidget *parent)
    : KXmlGuiWindow(parent), ui(new Ui::MainWindow) {
  ui->setupUi(this);
  m_instance = this;
  init();
}
...
void MainWindow::init() {
  ...
  if (firstRun()) {
    ...
    if (QMessageBox::information(
            this, "QFaktury", trUtf8("Czy chcesz skonfigurować firmę? Opcja ta "
    ...
    ui->filtrStart->setDisplayFormat(QLocale::ShortFormat);
    ui->filtrStart->setDate((QDate(QDate::currentDate().year(), 1, 1)));
    ui->filtrEnd->setDisplayFormat(QLocale::ShortFormat);
  ...
  actionCollection()->addAction("actionCreateBackup", ui->actionCreateBackup);
  actionCollection()->addAction("actionLoadBackup", ui->actionLoadBackup);
  actionCollection()->addAction("fileEndAction", ui->fileEndAction);

  actionCollection()->addAction("Faktura VAT", ui->addInvoiceAction);
  actionCollection()->addAction("invoiceGrossAction", ui->invoiceGrossAction);
  actionCollection()->addAction("invoiceRRAction", ui->invoiceRRAction);
  actionCollection()->addAction("invoiceBillAction", ui->invoiceBillAction);
  actionCollection()->addAction("Faktura Pro Forma", ui->invoiceProFormaAction);
  actionCollection()->addAction("invoiceCorrAction", ui->invoiceCorrAction);
  actionCollection()->addAction("invoiceDuplAction", ui->invoiceDuplAction);
  actionCollection()->addAction("invoiceDelAction", ui->invoiceDelAction);
  actionCollection()->addAction("invoiceEdAction", ui->invoiceEdAction);
  actionCollection()->addAction("sendEmailAction", ui->sendEmailAction);

  actionCollection()->addAction("Dodaj kontrahenta", ui->addBuyersAction);
  actionCollection()->addAction("delBuyersAction", ui->delBuyersAction);
  actionCollection()->addAction("editBuyersAction", ui->editBuyersAction);
  actionCollection()->addAction("actionPrintBuyer", ui->actionPrintBuyer);

  actionCollection()->addAction("Dodaj towar", ui->addGoodsAction);
  actionCollection()->addAction("delGoodsAction", ui->delGoodsAction);
  actionCollection()->addAction("editGoodsAction", ui->editGoodsAction);

  actionCollection()->addAction("Dane Firmy", ui->fileData_companyAction);
  actionCollection()->addAction("Faktura VAT", ui->addInvoiceAction);
  actionCollection()->addAction("Faktura Pro Forma", ui->invoiceProFormaAction);
  actionCollection()->addAction("Dodaj kontrahenta", ui->addBuyersAction);
  actionCollection()->addAction("Dodaj towar", ui->addGoodsAction);
  actionCollection()->addAction("Ustawienia", ui->fileSettingsAction);
  ...
  setCentralWidget(ui->widget_2);
  setupGUI(Default, ":/inne/ui/mainwindow_ui.rc");
}
...
void MainWindow::loadPlugins() {
  ...
  if (!files.isEmpty()) {
    ...
    for (constIterator = files.constBegin(); constIterator != files.constEnd();
      ...
      //   actionCollection()->addAction("Informacje", action);
  ...
  QPointer<QAction> action(actionCollection()->addAction(
      "Informacje", this, SLOT(pluginInfoSlot())));
...
void MainWindow::saveAllSett() {
  ...
  // saves filtr
  sett().setValue("filtrStart", ui->filtrStart->currentText());
  sett().setValue("filtrEnd", ui->filtrEnd->currentText());
...
void MainWindow::createBackup() {
  ...
  // two separates archives because of KArchive class methods, that add
  // directories and files with absolute paths to created archives -
  // addLocalFile and addLocalDirectory
  QFile arch1(sett().getWorkingDir() + QString("/") + QString("Backup-xml"));
  arch1.setPermissions(arch1.fileName(),
                       QFileDevice::WriteGroup | QFileDevice::ReadGroup |
                           QFileDevice::ReadUser | QFileDevice::WriteUser |
                           QFileDevice::ReadOwner | QFileDevice::WriteOwner);
  QFile arch2(sett().fileName().left(sett().fileName().lastIndexOf("/")) +
              QString("/") + QString("Backup-configs"));
  arch2.setPermissions(arch2.fileName(),
                       QFileDevice::WriteGroup | QFileDevice::ReadGroup |
                           QFileDevice::ReadUser | QFileDevice::WriteUser |
                           QFileDevice::ReadOwner | QFileDevice::WriteOwner);
  ...
  bool removed = false;
  if (arch1.exists())
    removed = arch1.remove();

  if (!removed)
    qDebug() << arch1.fileName() << " archive couldn't be removed";

  removed = false;
  if (arch2.exists())
    removed = arch2.remove();

  if (!removed)
    qDebug() << arch2.fileName() << " archive couldn't be removed";

  if (!arch1.open(QFile::ReadWrite))
    QMessageBox::warning(this, "warning",
                         arch1.error() + " || " + arch1.errorString());
  else
    arch1.close();

  if (!arch2.open(QFile::ReadWrite))
    QMessageBox::warning(this, "warning",
                         arch2.error() + " || " + arch2.errorString());
  else
    arch2.close();
  ...
  if (!openedAr)
    QMessageBox::warning(this, "warning",
                         "Archiwum nie może zostać otworzone do zapisu");
  if (!openedAr)
    QMessageBox::warning(this, "warning", kzip->errorString());

  bool addedDir =
      kzip->addLocalDirectory(sett().getWorkingDir(), sett().getWorkingDir());
  ...
  if (!closedOk)
    QMessageBox::warning(this, "warning",
                         "Zamknięcie archiwum zakończyło się niepowodzeniem");
  if (!closedOk)
    QMessageBox::warning(this, "warning", kzip->errorString());
  ...
  if (!openedAr2)
    QMessageBox::warning(this, "warning",
                         "Archiwum nie może zostać otworzone do zapisu");
  if (!openedAr2)
    QMessageBox::warning(this, "warning", kzip2->errorString());
  ...
  bool addedFile2 = kzip2->addLocalFile(
      sett().fileName().left(sett().fileName().lastIndexOf("/")) + "/user.conf",
  ...
  if (closedOk && closedOk2)
    QMessageBox::information(
        this, "Miejsce zapisu",
        QString("Został stworzony \"Backup-xml\" w ścieżce %1 oraz archiwum "
                "\"Backup-configs\" w ścieżce %2")
            .arg(QFileInfo(arch1.fileName()).absolutePath())
            .arg(QFileInfo(arch2.fileName()).absolutePath()));
  else
    QMessageBox::information(
        this, "Błąd zapisu",
        QString("Wystąpił błąd podczas zapisu. Treść ostatniego błędu jest "
                "następująca: %1, %2")
            .arg(kzip->errorString())
            .arg(kzip2->errorString()));
...
// idea from wysota's comment:
// http://www.qtcentre.org/threads/429-how-can-i-copy-a-directary-using-QDir
bool MainWindow::copyDir(const QString &src, const QString &dest) {
  ...
  for (QList<QFileInfo>::iterator it = entries.begin(); it != entries.end();
       ++it) {
    ...
    if (finfo.isSymLink()) { /* do something here */
      continue;
    }
...
void MainWindow::loadBackup() {
  ...
  QMessageBox::warning(this, "Wybór backup",
                       "Pamiętaj, że z uwagi na niebiezpieczeństwo "
                       "przechowywanych plików, zostaną odtworzone pliki, "
                       "które nie istnieją. Istniejące nie zostaną nadpisane.");

  QString fileName = sett().getWorkingDir() + QString("/Backup-xml");
  QString fileName2 =
      sett().fileName().left(sett().fileName().lastIndexOf("/")) +
      QString("/Backup-configs");
  ...
  // extracting path from root path is dangerous and needs permissions, so
  // extracting to config directories the archives and copying to original
  // config directory the files is what I've hit on
  bool ifCopied = archdir->copyTo(
      sett().fileName().left(sett().fileName().lastIndexOf("/")));

  if (!ifCopied)
    qDebug() << fileName << " hasn't been extracted";
  else
    qDebug() << fileName << " has been extracted";

  QDir dir2;
  dir2.setPath(sett().fileName().left(sett().fileName().lastIndexOf("/")) +
               sett().fileName().left(sett().fileName().lastIndexOf("/")));
  //    QMessageBox::warning(this,"",
  //    sett().fileName().left(sett().fileName().lastIndexOf("/")) +
  //    sett().fileName().left(sett().fileName().lastIndexOf("/")));
  foreach (QString copy_file, dir2.entryList(QDir::Files)) {
    QFile file(sett().fileName().left(sett().fileName().lastIndexOf("/")) +
               sett().fileName().left(sett().fileName().lastIndexOf("/")) +
               QString("/") + copy_file);
    file.copy(sett().fileName().left(sett().fileName().lastIndexOf("/")) +
              QString("/") + copy_file);
  }

  QDir dirscHome(sett().fileName().left(sett().fileName().lastIndexOf("/")) +
                 QString("/home"));
  bool removed = dirscHome.removeRecursively();
  if (!removed)
    qDebug() << dirscHome.absolutePath() << " directory couldn't be removed";
  else
    qDebug() << dirscHome.absolutePath() << " has been removed";
  ...
  //   QMessageBox::warning(this,"", sett().getWorkingDir() +
  //   sett().getWorkingDir());
  foreach (QString copy_file, dir.entryList(QDir::Dirs | QDir::Files))

  {

    QFile file(sett().getWorkingDir() + sett().getWorkingDir() + QString("/") +
               copy_file);
    QFileInfo ifDir(sett().getWorkingDir() + sett().getWorkingDir() +
                    QString("/") + copy_file);

    if (ifDir.isDir())
      copyDir(sett().getWorkingDir() + sett().getWorkingDir() + QString("/") +
                  copy_file,
              sett().getWorkingDir() + QString("/") + copy_file);
    else
      file.copy(sett().getWorkingDir() + QString("/") + copy_file);
  }

  QDir dirscHome2(sett().getWorkingDir() + QString("/home"));
  removed = dirscHome2.removeRecursively();
  if (!removed)
    qDebug() << dirscHome2.absolutePath() << " directory couldn't be removed";
  else
    qDebug() << dirscHome2.absolutePath() << " has been removed";

  QMessageBox::information(
      this, "Wczytywanie backup",
      "Sprawdź, restartując program, czy backup wprowadził brakujące dane");
