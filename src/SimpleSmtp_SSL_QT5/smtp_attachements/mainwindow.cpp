...
void EmailWindow::mailSent()
{
    if(smtp->getStatus() == "Message sent")
        QMessageBox::warning( new QWidget, trUtf8( "Qt Simple SMTP client" ), trUtf8( "Wiadomość została wysłana!\n\n" ) );
    else
        QMessageBox::warning(new QWidget, trUtf8( "Qt Simple SMTP client" ), trUtf8( "Wysłanie wiadomości nie było możliwe. Upewnij się, że poprawnie namieściłeś dane w rubrykach i jesteś podłączony do sieci." ));
}

const QLineEdit* EmailWindow::getServerName()
{
    return ui->server;
}

void EmailWindow::setServerName(QString server)
{
    ui->server->setText(server);
}

const QLineEdit* EmailWindow::getPortNumber()
{
    return ui->port;
}

void EmailWindow::setPortNumber(QString port)
{
    ui->port->setText(port);
}

const QLineEdit* EmailWindow::getUserName()
{
    return ui->uname;
}
...
const QLineEdit* EmailWindow::getUserPassword()
{
    return ui->paswd;
}
...
const QLineEdit* EmailWindow::getRecipMail()
{
    return ui->rcpt;
}
...
const QLineEdit* EmailWindow::getSubject()
{
    return ui->subject;
}

void EmailWindow::setSubject(QString subject)
{
    ui->subject->setText(subject);
}

const QPlainTextEdit* EmailWindow::getMessage()
{
    return ui->msg;
}
...
const QLineEdit* EmailWindow::getFiles()
{
   return ui->file;
}
...
void EmailWindow::addFile(QString file)
{
    files.append(file);
}

void EmailWindow::removeFile(QString rFile)
{
    files.removeOne(rFile);
}
...
const QRadioButton* EmailWindow::getTemplate_1()
{
    return ui->template1;
}

const QRadioButton* EmailWindow::getTemplate_2()
{
    return ui->template2;
}

const QRadioButton* EmailWindow::getTemplate_3()
{
    return ui->template3;
}
