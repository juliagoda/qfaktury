...
private:
  ...
  inline QuaZipFilePrivate(QuaZipFile *q)
      : q(q), zip(NULL), caseSensitivity(QuaZip::csDefault), raw(false),
        writePos(0), uncompressedSize(0), crc(0), internal(true),
        zipError(UNZ_OK) {}
  /// The constructor for the corresponding QuaZipFile constructor.
  inline QuaZipFilePrivate(QuaZipFile *q, const QString &zipName)
      : q(q), caseSensitivity(QuaZip::csDefault), raw(false), writePos(0),
        uncompressedSize(0), crc(0), internal(true), zipError(UNZ_OK) {
    zip = new QuaZip(zipName);
  }
  /// The constructor for the corresponding QuaZipFile constructor.
  inline QuaZipFilePrivate(QuaZipFile *q, const QString &zipName,
                           const QString &fileName, QuaZip::CaseSensitivity cs)
      : q(q), raw(false), writePos(0), uncompressedSize(0), crc(0),
  ...
  inline QuaZipFilePrivate(QuaZipFile *q, QuaZip *zip)
      : q(q), zip(zip), raw(false), writePos(0), uncompressedSize(0), crc(0),
        internal(false), zipError(UNZ_OK) {}
  /// The destructor.
...
QuaZipFile::QuaZipFile() : p(new QuaZipFilePrivate(this)) {}

QuaZipFile::QuaZipFile(QObject *parent)
    : QIODevice(parent), p(new QuaZipFilePrivate(this)) {}

QuaZipFile::QuaZipFile(const QString &zipName, QObject *parent)
    : QIODevice(parent), p(new QuaZipFilePrivate(this, zipName)) {}

QuaZipFile::QuaZipFile(const QString &zipName, const QString &fileName,
                       QuaZip::CaseSensitivity cs, QObject *parent)
    : QIODevice(parent), p(new QuaZipFilePrivate(this, zipName, fileName, cs)) {
}

QuaZipFile::QuaZipFile(QuaZip *zip, QObject *parent)
    : QIODevice(parent), p(new QuaZipFilePrivate(this, zip)) {}
...
QuaZip *QuaZipFile::getZip() const { return p->internal ? NULL : p->zip; }
...
void QuaZipFile::setZipName(const QString &zipName) {
  if (isOpen()) {
    qWarning("QuaZipFile::setZipName(): file is already open - can not set ZIP "
             "name");
    return;
  }
...
void QuaZipFilePrivate::setZipError(int zipError) const {
  QuaZipFilePrivate *fakeThis =
      const_cast<QuaZipFilePrivate *>(this); // non-const
...
bool QuaZipFile::open(OpenMode mode) { return open(mode, NULL); }

bool QuaZipFile::open(OpenMode mode, int *method, int *level, bool raw,
                      const char *password) {
  p->resetZipError();
  if (isOpen()) {
  ...
  if ((mode & ReadOnly) && !(mode & WriteOnly)) {
    if (p->internal) {
      ...
      if (p->zip->getMode() != QuaZip::mdUnzip) {
        qWarning("QuaZipFile::open(): file open mode %d incompatible with ZIP "
                 "open mode %d",
    ...
    p->setZipError(unzOpenCurrentFile3(p->zip->getUnzFile(), method, level,
                                       (int)raw, password));
  ...
  qWarning("QuaZipFile::open(): open mode %d not supported by this function",
           (int)mode);
  return false;
}

bool QuaZipFile::open(OpenMode mode, const QuaZipNewInfo &info,
                      const char *password, quint32 crc, int method, int level,
                      bool raw, int windowBits, int memLevel, int strategy) {
  zip_fileinfo info_z;
  p->resetZipError();
  if (isOpen()) {
    qWarning("QuaZipFile::open(): already opened");
  ...
  if ((mode & WriteOnly) && !(mode & ReadOnly)) {
    if (p->internal) {
      qWarning("QuaZipFile::open(): write mode is incompatible with internal "
               "QuaZip approach");
      return false;
    }
    if (p->zip == NULL) {
    ...
    if (p->zip->getMode() != QuaZip::mdCreate &&
        p->zip->getMode() != QuaZip::mdAppend &&
        p->zip->getMode() != QuaZip::mdAdd) {
      qWarning("QuaZipFile::open(): file open mode %d incompatible with ZIP "
               "open mode %d",
               (int)mode, (int)p->zip->getMode());
      return false;
    ...
    p->setZipError(zipOpenNewFileInZip3_64(
        p->zip->getZipFile(),
        ...
        method, level, (int)raw, windowBits, memLevel, strategy, password,
        (uLong)crc, p->zip->isZip64Enabled()));
    if (p->zipError == UNZ_OK) {
      p->writePos = 0;
      setOpenMode(mode);
      p->raw = raw;
...
bool QuaZipFile::isSequential() const { return true; }
...
qint64 QuaZipFile::csize() const {
  ...
  if (p->zip == NULL || p->zip->getMode() != QuaZip::mdUnzip)
    return -1;
  p->setZipError(unzGetCurrentFileInfo64(p->zip->getUnzFile(), &info_z, NULL, 0,
                                         NULL, 0, NULL, 0));
  if (p->zipError != UNZ_OK)
    return -1;
...
void QuaZipFile::close() {
  ...
  if (openMode() & ReadOnly)
    ...
    if (isRaw())
      p->setZipError(zipCloseFileInZipRaw64(p->zip->getZipFile(),
                                            p->uncompressedSize, p->crc));
    else
...
QString QuaZipFile::getFileName() const { return p->fileName; }

QuaZip::CaseSensitivity QuaZipFile::getCaseSensitivity() const {
  return p->caseSensitivity;
}

bool QuaZipFile::isRaw() const { return p->raw; }

int QuaZipFile::getZipError() const { return p->zipError; }

qint64 QuaZipFile::bytesAvailable() const { return size() - pos(); }
