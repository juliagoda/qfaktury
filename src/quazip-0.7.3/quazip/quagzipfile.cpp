...
class QuaGzipFilePrivate {
  ...
  bool open(FileId id, QIODevice::OpenMode mode, QString &error);
...
QuaGzipFile::QuaGzipFile() : d(new QuaGzipFilePrivate()) {}

QuaGzipFile::QuaGzipFile(QObject *parent)
    : QIODevice(parent), d(new QuaGzipFilePrivate()) {}

QuaGzipFile::QuaGzipFile(const QString &fileName, QObject *parent)
    : QIODevice(parent), d(new QuaGzipFilePrivate(fileName)) {}
...
QString QuaGzipFile::getFileName() const { return d->fileName; }

bool QuaGzipFile::isSequential() const { return true; }
...
bool QuaGzipFile::flush() { return gzflush(d->gzd, Z_SYNC_FLUSH) == Z_OK; }
