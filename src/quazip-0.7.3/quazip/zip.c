/* zip.c -- IO on .zip files using zlib
   ...
   archives
   Oct-2009 - Mathias Svensson - Did some code cleanup and refactoring to get
   ...
   Jan-2010 - back to unzip and minizip 1.0 name scheme, with compatibility
   layer
...
local int zip64local_getLong64
OF((const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream,
    ZPOS64_T *pX));
...
/************************************************************/
extern zipFile ZEXPORT zipOpen3(voidpf file, int append,
  ...
  ziinit.filestream =
      ZOPEN64(ziinit.z_filefunc, file,
...
extern zipFile ZEXPORT zipOpen2(voidpf file, int append,
                                zipcharpc *globalcomment,
...
extern zipFile ZEXPORT zipOpen2_64(voidpf file, int append,
                                   zipcharpc *globalcomment,
...
int Write_LocalFileHeader(zip64_internal *zi, const char *filename,
  ...
  if ((err == ZIP_OK) && (zi->ci.zip64)) {
    ...
    err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (short)HeaderID,
                              2);
    err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (short)DataSize,
                              2);
...
extern int ZEXPORT zipOpenNewFileInZip4_64(
  ...
  zi->ci.size_centralExtraFree =
      32; /* Extra space we have reserved in case we need to add ZIP64 extra
             info data */
  ...
  zip64local_putValue_inmemory(zi->ci.central_header + 4, (uLong)versionMadeBy,
                               2);
  zip64local_putValue_inmemory(zi->ci.central_header + 6,
                               (uLong)version_to_extract, 2);
  ...
  if ((err == Z_OK) && (password != NULL)) {
    ...
    if (crcForCrypting == 0) {
      crcForCrypting = (uLong)zi->ci.dosDate << 16; /* ATTANTION! Without this
                                                       row, you don't unpack
                                                       your password protected
                                                       archive in other app. */
    }
...
int Write_EndOfCentralDirectoryRecord(zip64_internal *zi, uLong size_centraldir,
                                      ZPOS64_T centraldir_pos_inzip) {
  int err = ZIP_OK;
