...
QuaZipDir::QuaZipDir(const QuaZipDir &that) : d(that.d) {}
...
QString QuaZipDir::operator[](int pos) const { return entryList().at(pos); }
...
bool QuaZipDir::cd(const QString &directoryName) {
  ...
  if (dirName.contains('/')) {
    ...
    if (dirName.startsWith('/')) {
      ...
      qDebug("QuaZipDir::cd(%s): going to /", dirName.toUtf8().constData());
#endif
      if (!dir.cd("/"))
        return false;
    }
...
#ifdef QUAZIP_QUAZIPDIR_DEBUG
      qDebug("QuaZipDir::cd(%s): going to %s", dirName.toUtf8().constData(),
...
bool QuaZipDir::cdUp() { return cd(".."); }

uint QuaZipDir::count() const { return entryList().count(); }

QString QuaZipDir::dirName() const { return QDir(d->dir).dirName(); }
...
public:
  ...
  inline ~QuaZipDirRestoreCurrent() { zip->setCurrentFile(currentFile); }
...
bool QuaZipDirPrivate::entryInfoList(QStringList nameFilters,
  ...
  if (srt != QDir::NoSort && (srt & QDir::Unsorted) != QDir::Unsorted) {
    if (QuaZip::convertCaseSensitivity(caseSensitivity) == Qt::CaseInsensitive)
      srt |= QDir::IgnoreCase;
...
QList<QuaZipFileInfo64>
QuaZipDir::entryInfoList64(const QStringList &nameFilters,
  ...
  if (fileName.contains('/')) {
    ...
    if (fileName == "..") {
      ...
      for (QStringList::const_iterator i = entries.constBegin();
        ...
        qDebug("QuaZipDir::exists(): entry: %s", i->toUtf8().constData());
      }
#endif
...
bool QuaZipDir::exists() const { return QuaZipDir(d->zip).exists(d->dir); }

QString QuaZipDir::filePath(const QString &fileName) const {
  return QDir(d->dir).filePath(fileName);
}

QDir::Filters QuaZipDir::filter() { return d->filter; }

bool QuaZipDir::isRoot() const { return d->simplePath().isEmpty(); }

QStringList QuaZipDir::nameFilters() const { return d->nameFilters; }

QString QuaZipDir::path() const { return d->dir; }
...
void QuaZipDir::setFilter(QDir::Filters filters) { d->filter = filters; }
...
void QuaZipDir::setSorting(QDir::SortFlags sort) { d->sorting = sort; }

QDir::SortFlags QuaZipDir::sorting() const { return d->sorting; }
