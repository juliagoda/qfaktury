...
typedef voidpf(ZCALLBACK *open_file_func)
    OF((voidpf opaque, voidpf file, int mode));
...
typedef int(ZCALLBACK *seek_file_func)
...
typedef int(ZCALLBACK *seek64_file_func)
...
//#define ZSEEK64(filefunc,filestream,pos,mode)   ((*((filefunc).zseek64_file))
...
voidpf call_zopen64 OF((const zlib_filefunc64_32_def *pfilefunc, voidpf file,
                        int mode));
int call_zseek64 OF((const zlib_filefunc64_32_def *pfilefunc, voidpf filestream,
