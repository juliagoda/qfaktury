...
private:
  ...
  /// The last error.
  int zipError;
  /// Whether \ref QuaZip::setDataDescriptorWritingEnabled() "the data
  /// descriptor writing mode" is enabled.
  ...
  inline QuaZipPrivate(QuaZip *q)
      : q(q), fileNameCodec(getDefaultFileNameCodec()),
        commentCodec(QTextCodec::codecForLocale()), ioDevice(NULL),
        mode(QuaZip::mdNotOpen), hasCurrentFile_f(false), zipError(UNZ_OK),
        dataDescriptorWritingEnabled(true), zip64(false), autoClose(true) {
    unzFile_f = NULL;
    zipFile_f = NULL;
  ...
  inline QuaZipPrivate(QuaZip *q, const QString &zipName)
      : q(q), fileNameCodec(getDefaultFileNameCodec()),
        commentCodec(QTextCodec::codecForLocale()), zipName(zipName),
        ioDevice(NULL), mode(QuaZip::mdNotOpen), hasCurrentFile_f(false),
        zipError(UNZ_OK), dataDescriptorWritingEnabled(true), zip64(false),
        autoClose(true) {
    unzFile_f = NULL;
    zipFile_f = NULL;
    lastMappedDirectoryEntry.num_of_file = 0;
  ...
  inline QuaZipPrivate(QuaZip *q, QIODevice *ioDevice)
      : q(q), fileNameCodec(getDefaultFileNameCodec()),
        commentCodec(QTextCodec::codecForLocale()), ioDevice(ioDevice),
        mode(QuaZip::mdNotOpen), hasCurrentFile_f(false), zipError(UNZ_OK),
        dataDescriptorWritingEnabled(true), zip64(false), autoClose(true) {
    unzFile_f = NULL;
    zipFile_f = NULL;
...
QuaZip::QuaZip() : p(new QuaZipPrivate(this)) {}

QuaZip::QuaZip(const QString &zipName) : p(new QuaZipPrivate(this, zipName)) {}

QuaZip::QuaZip(QIODevice *ioDevice) : p(new QuaZipPrivate(this, ioDevice)) {}
...
int QuaZip::getEntriesCount() const {
  ...
  if ((fakeThis->p->zipError = unzGetGlobalInfo64(p->unzFile_f, &globalInfo)) !=
      UNZ_OK)
    return p->zipError;
...
QString QuaZip::getComment() const {
  ...
  if ((fakeThis->p->zipError = unzGetGlobalComment(p->unzFile_f, comment.data(),
                                                   comment.size())) < 0)
    return QString();
  fakeThis->p->zipError = UNZ_OK;
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64 *info) const {
  ...
  if (!isOpen() || !hasCurrentFile())
    return false;
  if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
  ...
  if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
           p->unzFile_f, NULL, fileName.data(), fileName.size(), extra.data(),
           extra.size(), comment.data(), comment.size())) != UNZ_OK)
    return false;
  ...
  info->extra = extra;
  info->dateTime =
      QDateTime(QDate(info_z.tmu_date.tm_year, info_z.tmu_date.tm_mon + 1,
                      info_z.tmu_date.tm_mday),
                QTime(info_z.tmu_date.tm_hour, info_z.tmu_date.tm_min,
                      info_z.tmu_date.tm_sec));
...
QString QuaZip::getCurrentFileName() const {
  ...
  if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
           p->unzFile_f, NULL, fileName.data(), fileName.size(), NULL, 0, NULL,
           0)) != UNZ_OK)
    return QString();
...
QTextCodec *QuaZip::getFileNameCodec() const { return p->fileNameCodec; }
...
QTextCodec *QuaZip::getCommentCodec() const { return p->commentCodec; }

QString QuaZip::getZipName() const { return p->zipName; }
...
QuaZip::Mode QuaZip::getMode() const { return p->mode; }

bool QuaZip::isOpen() const { return p->mode != mdNotOpen; }

int QuaZip::getZipError() const { return p->zipError; }

void QuaZip::setComment(const QString &comment) { p->comment = comment; }

bool QuaZip::hasCurrentFile() const { return p->hasCurrentFile_f; }

unzFile QuaZip::getUnzFile() { return p->unzFile_f; }

zipFile QuaZip::getZipFile() { return p->zipFile_f; }
...
template <> QuaZipFileInfo QuaZip_getFileInfo(QuaZip *zip, bool *ok) {
  QuaZipFileInfo info;
  *ok = zip->getCurrentFileInfo(&info);
  return info;
}

template <> QuaZipFileInfo64 QuaZip_getFileInfo(QuaZip *zip, bool *ok) {
  QuaZipFileInfo64 info;
  *ok = zip->getCurrentFileInfo(&info);
  return info;
}

template <> QString QuaZip_getFileInfo(QuaZip *zip, bool *ok) {
...
void QuaZip::setZip64Enabled(bool zip64) { p->zip64 = zip64; }

bool QuaZip::isZip64Enabled() const { return p->zip64; }

bool QuaZip::isAutoClose() const { return p->autoClose; }

void QuaZip::setAutoClose(bool autoClose) const { p->autoClose = autoClose; }
