...
public:
  ...
  bool open(OpenMode mode, const QuaZipNewInfo &info,
            ...
            int method = Z_DEFLATED, int level = Z_DEFAULT_COMPRESSION,
            bool raw = false, int windowBits = -MAX_WBITS,
            int memLevel = DEF_MEM_LEVEL, int strategy = Z_DEFAULT_STRATEGY);
   ...
   * \ref quazipfile-sequential "QuaZipFile is considered to be sequential
   *device",
