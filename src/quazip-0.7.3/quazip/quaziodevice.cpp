...
QuaZIODevicePrivate::QuaZIODevicePrivate(QIODevice *io)
    : io(io), inBuf(NULL), inBufPos(0), inBufSize(0), outBuf(NULL),
      outBufPos(0), outBufSize(0), zBufError(false), atEnd(false) {
...
QuaZIODevice::QuaZIODevice(QIODevice *io, QObject *parent)
    : QIODevice(parent), d(new QuaZIODevicePrivate(io)) {
  ...
  while (read < maxSize) {
    ...
    while (read < maxSize && d->inBufPos < d->inBufSize) {
      ...
      case Z_BUF_ERROR: // this should never happen, but just in case
        ...
        d->inBufPos = 0;
        more = d->io->read(d->inBuf + d->inBufSize,
                           QUAZIO_INBUFSIZE - d->inBufSize);
        if (more == -1) {
          setErrorString(d->io->errorString());
          return -1;
        }
...
bool QuaZIODevice::isSequential() const { return true; }
...
bool QuaZIODevice::atEnd() const {
  ...
  return (openMode() == NotOpen) ||
         (QIODevice::bytesAvailable() == 0 && d->atEnd);
}
