...
Validations::Validations(QObject *parent)
	: QObject(parent)
{
	m_instance = this;
}
...
bool Validations::validateAccount(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany rachunek bankowy. Wymagany format to "
								  "\"SSXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX\" lub "
								  "\"XXXXXXXXXXXXXXXXXXXXXXXXXX\" lub \"XX XXXX XXXX XXXX XXXX "
								  "XXXX XXXX\" lub \"SSXX XXXX XXXX XXXX XXXX XXXX XXXX\"  gdzie X "
								  "oznacza cyfrę (zawsze jest 26 X), a S oznacza dużą literę"));
...
bool Validations::validateEmail(QString text)
{
	...
	// dot ., provided that it is not the first or last character unless quoted, and provided also
	// that it does not appear consecutively unless quoted (e.g. John..Doe@example.com is not
	// allowed but "John..Doe"@example.com is allowed);
	// space and "(),:;<>@[\] characters are allowed with restrictions (they are only allowed inside
	// a quoted string, as described in the paragraph below, and in addition, a backslash or
	// double-quote must be preceded by a backslash);
	// comments are allowed with parentheses at either end of the local-part; e.g.
	// john.smith(comment)@example.com and (comment)john.smith@example.com are both equivalent to
	...
	if (!masks.exactMatch(text) || !notTwoDotted)
	{
		QMessageBox::warning(
			...
			trUtf8("Źle podany adres email. Dozwolone są duże litery, małe litery, cyfry, "
				   ...
				   "na początku lub na końcu, a jak już to w znakach \"\"), znak - (o ile nie "
				   "znajduje się na początku lub końcu) oraz znaki specjalne jak "
...
bool Validations::validateIDCard(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany numer dowodu osobistego. Wymagany format dowodu to "
...
{
	QRegExp masks(
		"^(([0-9]{3})-([0-9]{2})-([0-9]{2})-([0-9]{3}))|(([0-9]{3})-([0-9]{3})-([0-9]{2})-([0-9]{2}"
		"))$");

	if (!masks.exactMatch(text))
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany numer NIP. Wymagany format NIP to XXX-XXX-XX-XX lub "
...
bool Validations::validatePass(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany numer seryjny paszportu. Wymagany format dowodu to "
...
bool Validations::validatePESEL(QString text)
{
	...
	if (!masks.exactMatch(text))
	{
		QMessageBox::warning(0, "QFaktury", trUtf8("Źle podany PESEL. Wymagany format to "
...
bool Validations::validatePkwiu(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany numer PKWiU. Wymagany format to XX(.XX.XX.X) , gdzie "
								  "X oznacza cyfrę, a część w nawiasie oznacza opcjonalną postać. "
								  "Przykładowo więc może to też być XX.X czy XX.XX.XX"));
...
bool Validations::validateTel(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany numer telefonu. Wymagany format to +XX(X od 5 do 20) "
...
bool Validations::validateWebsite(QString text)
{
	...
	QRegExp masks(
		"^(https?://)?(www\\.)?(([0-9a-z]+)((-)*))+(\\.){1}((([a-z0-9]+)([" + specialChar
		+ "]*)((/)*)((\\.)*))+)$");

	if (!masks.exactMatch(text))
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Źle podany adres internetowy. Format pozwala na umieszczenie "
...
bool Validations::checkSumNIP(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Weryfikator cyfry kontrolnej NIP wykrył niepoprawny numer NIP. "
...
bool Validations::checkSumREGON(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Weryfikator cyfry kontrolnej REGON wykrył niepoprawny numer. "
								  "Sprawdź kolejność wpisanych liczb."));
		return false;
	}
}
...
bool Validations::checkSumPESEL(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Weryfikator cyfry kontrolnej PESEL wykrył niepoprawny numer. "
								  "Sprawdź kolejność wpisanych liczb."));
		return false;
	}
}
...
bool Validations::checkSumAccount(QString text)
{
	if (symbols().contains(text.at(0)) && symbols().contains(text.at(1)))
	{
		...
		{
			QMessageBox::warning(
				0, "QFaktury", trUtf8("Weryfikator konta bankowego IBAN wykrył niepoprawny numer. "
									  "Sprawdź kolejność wpisanych liczb."));
			return false;
		}
	}
...
bool Validations::checkSumIDCard(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Weryfikator polskiego dowodu osobistego wykrył niepoprawny "
								  "numer. Sprawdź kolejność wpisanych znaków. Możesz także zgłosić "
								  "błąd numeru dowodu w odpowiednim Urzędzie Miasta lub Gminy."));
		return false;
	}
}
...
bool Validations::checkSumPass(QString text)
{
	...
	{
		QMessageBox::warning(
			0, "QFaktury", trUtf8("Weryfikator numeru seryjnego paszportu wykrył niepoprawny "
								  "numer. Sprawdź kolejność wpisanych znaków. Możesz także zgłosić "
								  "błąd numeru paszportu w najbliższym Urzędzie Wojewódzkim."));
		return false;
	}
}
