...
void User::init () {
  ...
  for (int i = 1; i < settings.value ("sellerCount").toInt(); ++i) {
    ...
    sellersWidgets.at(i)->findChild<QLineEdit*>("nameEdit")->setText(settings.value ("name").toString());	// save String
    sellersWidgets.at(i)->findChild<QLineEdit*>("placeEdit")->setText(settings.value ("city").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("codeEdit")->setText(settings.value ("zip").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("addressEdit")->setText(settings.value ("address").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("accountEdit")->setText(settings.value ("account").toString());

    if (!settings.value ("secIdType").isNull ()) {
       int current = sellersWidgets.at(i)->findChild<QComboBox*>("secIdType")->findText(settings.value ("secIdType").toString());
       sellersWidgets.at(i)->findChild<QComboBox*>("secIdType")->setCurrentIndex (current);
    }

    sellersWidgets.at(i)->findChild<QLineEdit*>("regonEdit")->setText(settings.value ("regon").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("phonEdit")->setText(settings.value ("phone").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("emailEdit")->setText(settings.value ("email").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("webEdit")->setText(settings.value ("website").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("krsEdit")->setText(settings.value ("krs").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("faxEdit")->setText(settings.value ("fax").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("bankNameEdit")->setText(settings.value ("bank").toString());
    sellersWidgets.at(i)->findChild<QLineEdit*>("bicSwiftEdit")->setText(settings.value ("swift").toString());
    if (settings.value ("ifUsed").toBool()) sellersWidgets.at(i)->findChild<QCheckBox*>()->setChecked(true);
    else sellersWidgets.at(i)->findChild<QCheckBox*>()->setChecked(false);
    settings.endGroup();
...
bool User::checkAll()
{
    ...
    {
        if (Validations::instance()->isEmptyField(widg->findChild<QLineEdit*>("nameEdit")->text(),textLabel1->text())) return false;
    }

    foreach (QWidget* widg, sellersWidgets)
    {
        if (Validations::instance()->isEmptyField(widg->findChild<QLineEdit*>("placeEdit")->text(),textLabel3->text())) return false;
    }

    foreach (QWidget* widg, sellersWidgets)
    {
        if (!Validations::instance()->isEmptyField(widg->findChild<QLineEdit*>("codeEdit")->text(),textLabel4->text())) {
            if (!Validations::instance()->validateZip(widg->findChild<QLineEdit*>("codeEdit")->text())) return false;
           } else {
            return false;
            }
    }
    ...
    foreach (QWidget* widg, sellersWidgets)
    {
    if (Validations::instance()->isEmptyField(widg->findChild<QLineEdit*>("addressEdit")->text(),textLabel2->text())) return false;
    }

    foreach (QWidget* widg, sellersWidgets)
    {
    if (!Validations::instance()->isEmptyField(widg->findChild<QLineEdit*>("accountEdit")->text(),textLabel5->text())) {
        if (!Validations::instance()->validateAccount(widg->findChild<QLineEdit*>("accountEdit")->text())) return false;
        if (!Validations::instance()->checkSumAccount(widg->findChild<QLineEdit*>("accountEdit")->text())) return false;
        } else {
        return false;
        }
    }
    ...
    case 2:
        ...
        if (!Validations::instance()->validateRegon(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
        if (!Validations::instance()->checkSumREGON(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
            break;

    case 3:

        if (!Validations::instance()->validatePESEL(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
        if (!Validations::instance()->checkSumPESEL(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
            break;

    case 4:

        if (!Validations::instance()->validateIDCard(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
        if (!Validations::instance()->checkSumIDCard(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
            break;

    case 5:

        if (!Validations::instance()->validatePass(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
        if (!Validations::instance()->checkSumPass(widg->findChild<QLineEdit*>("regonEdit")->text())) return false;
            break;
        }
    }
    }
    ...
    if (!widg->findChild<QLineEdit*>("phonEdit")->text().isEmpty()) {
        if (!Validations::instance()->validateTel(widg->findChild<QLineEdit*>("phonEdit")->text())) return false;
        }
    }

    foreach (QWidget* widg, sellersWidgets) {
    if (!widg->findChild<QLineEdit*>("emailEdit")->text().isEmpty()) {
        if (!Validations::instance()->validateEmail(widg->findChild<QLineEdit*>("emailEdit")->text())) return false;
        }
    }

    foreach (QWidget* widg, sellersWidgets) {
    if (!widg->findChild<QLineEdit*>("webEdit")->text().isEmpty()) {
        if (!Validations::instance()->validateWebsite(widg->findChild<QLineEdit*>("webEdit")->text())) return false;
        }
    }
  ...
  if (sett().value("validation").toBool() == false || (sett().value("validation").toBool() && checkAll())) {
      ...
      foreach (QWidget* widg, sellersWidgets)
      {
          if (i == 0) {
              ...
              QSettings settings("elinux", "user");
              settings.beginGroup("seller" + QString::number(i));
              settings.setValue ("name", widg->findChild<QLineEdit*>("nameEdit")->text ());	// save String
              ...
              settings.setValue ("secIdType", widg->findChild<QComboBox*>("secIdType")->currentText ());
...
void User::on_nextSeller_clicked()
{
    ...
    QStringList listNames = QStringList() << "Nazwa*:" << "Miejscowość*:" << "Kod*:" << "Adres*:" << "Nr konta*:"
                                      << "Nazwa banku:" << "Nr SWIFT/BIC:" << "NIP*:" << "Typ 2 Id:" << "Drugi Id:"
                                      << "Numer KRS:" << "Telefon:" << "Fax:" << "Email:" << "Strona www:";

    QStringList objectNames = QStringList() << "nameEdit" << "placeEdit" << "codeEdit" << "addressEdit" << "accountEdit"
                                            << "bankNameEdit" << "bicSwiftEdit" << "nipEdit" << "secIdType" << "regonEdit"
                                            << "krsEdit" << "phonEdit" << "faxEdit" << "emailEdit" << "webEdit";
    ...
    for (int i = 0; i < listNames.count(); i++)
    {
        ...
        {
            QStringList listcombo = QStringList() << "Brak" << "NUSP" << "REGON"
...
{
    if (QMessageBox::warning(this,trUtf8("Usuwanie danych oddziału"),trUtf8("Usunąć dane kolejnego sprzedawcy/oddziału?"), trUtf8("Tak"), trUtf8("Nie"), 0, 0,	1) == 0)
    {
        QSettings settings("elinux", "user");
        settings.remove("seller" + QString::number(sellersList->currentIndex()));

        bool isLess = false;
        if (sellersList->currentIndex() < sellersList->count()-1) isLess = true;
        ...
        if (isLess)
        {
            ...
            for (int i = delIndex, j = delIndex + 1; i < sellersList->count(), j <= sellersList->count(); ++i, ++j)
            {
